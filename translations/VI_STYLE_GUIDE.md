...
| 金币 | Vàng | |
| 绑定金币 | Xu khóa | |
| 经验 | KN | Dùng ở nhãn/chỉ số/phần thưởng; văn kể dài có thể dùng `kinh nghiệm` |
| 等级 | Cấp | |
...
### Quy ước phần thưởng và số lớn

- Nhãn phần thưởng phải dùng nhất quán: `KN`, `Vàng`, `Xu khóa`, `Nguyên Bảo`.
- Số lượng thưởng từ `10.000` trở lên hiển thị hậu tố `K`; ví dụ `10.000 → 10K`, `35.083 → 35.1K`. Chỉ rút gọn ở lớp hiển thị, không đổi số liệu server.
- Câu thoại, mô tả dài và hướng dẫn có thể dùng từ thường `kinh nghiệm` để câu tự nhiên; không viết lại payload định tuyến hoặc tên tiền tệ trong dữ liệu máy.
