...
## Sự cố chuỗi ABC ngày 15/07/2026

Bản `f41` dùng cách thay chuỗi thô đã làm tag `DoABC` tại offset `34598` vẫn giữ độ dài cũ, lớn hơn phần dữ liệu còn lại. SWF vẫn giải nén được nhưng Flash ActiveX không thể nạp, dẫn đến màn hình đen.

Quy trình thay chuỗi bắt buộc:

- Chỉ thay hằng chuỗi đã được xác định trong string pool của DoABC; cập nhật đồng thời U30, độ dài DoABC và độ dài file FWS.
- Kiểm tra container/tag và toàn bộ ABC string pool trước và sau khi vá.
- Nếu tạo CWS, nén rồi giải nén lại bằng FFDec và so sánh byte-for-byte với FWS đã kiểm tra.
- Không dùng vá string-pool cho biểu thức có số literal nguồn/đích khác nhau. Các key này phải đi qua quy trình P-code/bytecode đầy đủ trên đúng baseline.
...
- [ ] Với patch string pool: FWS trước/sau có End tag đúng, DoABC không vượt dữ liệu và CWS round-trip khớp FWS.
- [ ] Key nhiều literal không bị ép vào string pool; đã tách sang quy trình P-code/bytecode đầy đủ.
