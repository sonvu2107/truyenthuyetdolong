...
$workRoot = Join-Path 'C:\Temp' ('ahtl-github-sync-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    foreach ($file in $manifest.files) {
        $source = [IO.Path]::Combine($workRoot, $file.source.Replace('/', '\'))
        $target = [IO.Path]::Combine($WebRoot, $file.target.Replace('/', '\'))
        $backup = [IO.Path]::Combine($backupRoot, $file.target.Replace('/', '\'))
